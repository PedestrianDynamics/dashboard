...
class data_files:
    ...
    default_geometry_file: str = "geometry.xml"

    def process_traj_file(self) -> StringIO:
        """return string data for traj"""
        if self.uploaded_traj_file:
            stringio = StringIO(self.uploaded_traj_file.getvalue().decode("utf-8"))
            string_data = stringio.read()
        else:  # if self.selected_traj_file:
    ...
    def process_geo_file(self) -> StringIO:
